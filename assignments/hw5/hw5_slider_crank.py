...
## Slider (as a box)
slider = chrono.ChBodyEasyBox(0.2, 0.2, 0.2, 1000, True, True)
slider.SetPos(chrono.ChVector3d(1 + np.sqrt(3), 0, 0))
slider.SetRot(chrono.ChQuaterniond(1, 0, 0, 0))
slider.SetName("slider")
system.Add(slider)
slider.GetVisualShape(0).SetColor(chrono.ChColor(0.2, 0.2, 0.6))
...
# create a joint between rod and slider
joint_b = chrono.ChLinkLockSpherical()  # or other joint type
joint_b.SetName("joint_b")
joint_b.Initialize(rod, 
                   slider, 
                   chrono.ChFramed(chrono.ChVector3d(1+np.sqrt(3), 0, 0)))
system.AddLink(joint_b)

# Joint between ground and slider
joint_c = chrono.ChLinkLockPrismatic()  # or other joint type
joint_c.SetName("joint_c")
joint_c.Initialize(slider,
                   ground, 
                   chrono.ChFramed(chrono.ChVector3d(1 + np.sqrt(3), 0, 0), chrono.Q_ROTATE_Z_TO_X))
